...
from dolma.cli import BaseCli, field, print_config, DolmaConfigError
from dolma.core.loggers import get_logger

from .inference import run_inference
from .utils import is_valid_device, is_valid_dtype, is_valid_jq_expr
...
class DolmaClassifierInferenceCli(BaseCli):
    ...
    def run(cls, parsed_config: DolmaClassifierInferenceConfig):
        ...
        destination = [str(dest) for dest in parsed_config.destination] if parsed_config.destination else None

        if len(documents) == 0:
            raise DolmaConfigError("At least one documents path must be specified")

        if destination is not None and len(destination) != len(documents):
            raise DolmaConfigError("Number of documents and destinations must match")
        elif destination is None and any("/documents/" not in doc for doc in documents):
        ...
        if not is_valid_dtype(parsed_config.model.dtype):
            raise DolmaConfigError(f"Invalid dtype: {parsed_config.model.dtype}")

        if not is_valid_device(parsed_config.model.device):
            raise DolmaConfigError(f"Invalid device: {parsed_config.model.device}")
        ...
        run_inference(
            ...
            ignore_existing=bool(parsed_config.ignore_existing),
