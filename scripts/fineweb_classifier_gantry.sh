...
gantry run \
    ...
    --yes \
    -- /bin/bash -c "huggingface-cli download ${MODEL_NAME} && torchrun --standalone --nproc_per_node=8 scripts/fineweb_classifier.py --source-prefix ${DOCUMENTS} --batch-size 1024 --model-name ${MODEL_NAME}"
